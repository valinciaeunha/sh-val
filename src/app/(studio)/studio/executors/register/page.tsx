...
import { createExecutor, CreateExecutorData } from "@/lib/api/executors";
import { authApi } from "@/lib/api/auth";

export default function ExecutorRegistrationPage() {
            ...
            <div className="max-w-md mx-auto py-16 text-center space-y-6 animate-in fade-in zoom-in-95 duration-700">
                ...
                <div className="space-y-3">
                    ...
                    <p className="text-sm font-mono text-offgray-400 leading-relaxed max-w-[90%] mx-auto">
                        ...
                        <br />
                        Please log out and sign in using Discord OAuth to register your Executor.
                    </p>
                </div>
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const executorData: CreateExecutorData = {
                name,
                description,
                website,
                discord,
            ...
            if (bannerPreview) {
                executorData.banner = dataURLtoBlob(bannerPreview);
            }
            if (logoPreview) {
                executorData.logo = dataURLtoBlob(logoPreview);
            }

            await createExecutor(executorData);
            ...
            if (error.error === "ValidationError" && Array.isArray(error.details)) {
                ...
                alert(error.message || "Failed to register executor. Please try again.");
            }
        } finally {
            setIsLoading(false);
        }
    ...
    if (isSuccess) {
        return (
            <div className="max-w-2xl mx-auto py-12 px-6 bg-[#11141A] border border-[#10B981]/10 rounded-xl relative overflow-hidden flex flex-col items-center justify-center text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                <h2 className="text-2xl font-serif text-offgray-50 mb-3 relative z-10">Application Submitted</h2>
        ...
        <div className="max-w-2xl mx-auto space-y-8 pb-12 animate-in fade-in duration-700">
            ...
            <div>
                ...
                <section className="space-y-1">
                    <h1 className="text-3xl font-serif tracking-tight text-offgray-50">Register Executor</h1>
                    <p className="text-sm font-mono text-offgray-500">
                        Create an official listing for your Roblox executor software.
                    </p>
                </section>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
                ...
                <section className="space-y-6">
                    <h2 className="text-xs font-mono tracking-widest uppercase text-offgray-400 flex items-center gap-3">
                        <span className="text-[#10B981]">04 /</span> Initial Version
                        <div className="h-px flex-1 bg-gradient-to-r from-white/[0.06] to-transparent" />
                    </h2>

                    <div className="bg-[#11141a] border border-white/[0.04] p-6 rounded-xl space-y-6">
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-mono text-offgray-500 uppercase tracking-widest block">
                                Patch Notes / Changelog <span className="text-offgray-600 lowercase tracking-normal">(optional)</span>
                            </label>
                            <textarea
                                rows={4}
                                value={patchNotes}
                                onChange={(e) => setPatchNotes(e.target.value)}
                                placeholder="What's new in this initial release?"
                                className="w-full px-4 py-3 bg-[#0a0c10] border border-white/[0.04] hover:border-white/[0.08] focus:border-[#10B981]/40 focus:ring-1 focus:ring-[#10B981]/20 rounded-lg text-sm font-mono tracking-tight text-offgray-100 placeholder-offgray-600 focus:outline-none transition-all resize-none shadow-inner"
                            />
                        </div>
                    </div>
