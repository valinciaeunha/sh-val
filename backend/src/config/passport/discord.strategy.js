  ...
  passport.use(
    new DiscordStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        try {
          // Auto-join Discord Server if configured
          if (config.discord.botToken && config.discord.guildId) {
            try {
              await fetch(`https://discord.com/api/v10/guilds/${config.discord.guildId}/members/${profile.id}`, {
                method: 'PUT',
                headers: {
              ...
              console.error('Failed to auto-join Discord server:', err);
              // Do not fail the login if auto-join fails
            }
          }
